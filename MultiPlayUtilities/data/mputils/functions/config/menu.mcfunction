tellraw @s [{"text":"設定をクリックで選んでください。\n\n"},{"text":"発光モード: "},{"text":"ON","color":"green","underlined":true,"clickEvent":{"action":"run_command","value":"/data merge storage para7sys:mpu {glow: true}"}},{"text":" / "},{"text":"OFF","color":"red","underlined":true,"clickEvent":{"action":"run_command","value":"/data merge storage para7sys:mpu {glow: false}"}}]
