...
execute at @e[type=#para7_utils:enemymob,sort=nearest,limit=1] run summon creeper ~ ~ ~ {NoAI:1b,powered:1b,ExplosionRadius:5b,Fuse:0,CustomName:'{"text":"ディザスター"}',Attributes:[{Name:generic.attackDamage,Base:4}]}
execute at @e[type=#para7_utils:enemymob,sort=nearest,limit=1] run summon lightning_bolt ~ ~ ~
