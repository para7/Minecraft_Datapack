execute if entity @e[type=#para7_utils:friendmob,distance=0.1..30,tag=!PJDummy,sort=nearest,limit=1] run tellraw @a[distance=..40] [{"selector":"@s"},{"text":"は"},{"selector":"@e[type=#para7_utils:friendmob,distance=0.1..30,tag=!PJDummy,sort=nearest,limit=1]"},{"text":"のところへワープした"}]
...
execute if entity @e[type=#para7_utils:friendmob,distance=0.1..30,tag=!PJDummy,sort=nearest,limit=1] run playsound minecraft:entity.enderman.teleport hostile @a ~ ~ ~ 1 1
