execute if entity @e[type=#para7_utils:enemymob_notboss,distance=0.1..30,tag=!PJDummy,sort=nearest,limit=1] run tellraw @a[distance=..40] [{"selector":"@s"},{"text":"は"},{"selector":"@e[type=#para7_utils:enemymob_notboss,distance=0.1..25,tag=!PJDummy,sort=nearest,limit=1] run tellraw @a[distance=..25]"},{"text":"と入れ替わった！"}]
